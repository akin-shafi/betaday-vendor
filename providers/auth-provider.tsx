...
interface AuthContextType {
  ...
  login: (data: VendorLoginData & { rememberMe?: boolean }) => Promise<void>
  verifyOTP: (
    phoneNumber: string,
    otp: string,
    source: "login" | "signup",
  ) => Promise<{ success: boolean; data?: Vendor }>
  resendOTP: (phoneNumber: string, source: "login" | "signup") => Promise<{ success: boolean }>
  forgotPassword: (identifier: string) => Promise<void>
  resetPassword: (identifier: string, otp: string, newPassword: string) => Promise<void>
  logout: () => void
  updateProfile: (profileData: ProfileData) => Promise<{ success: boolean; data: Vendor }>
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signup = async (data: VendorSignupData) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || `HTTP ${response.status}: Signup failed`)
      }
      ...
      await router.push(`/auth/verify-otp?phone=${encodeURIComponent(data.phone.trim())}`)
    } catch (error) {
  ...
  const login = async (data: VendorLoginData & { rememberMe?: boolean }) => {
    setIsLoading(true)
    try {
  ...
  const verifyOTP = async (phoneNumber: string, otp: string, source: "login" | "signup") => {
    try {
      setIsLoading(true)
      const endpoint = source === "login" ? `${baseUrl}/users/login/phone` : `${baseUrl}/users/verify-signup-otp`
  ...
  const resendOTP = async (phoneNumber: string, source: "login" | "signup") => {
    try {
      setIsLoading(true)
      const endpoint = source === "login" ? `${baseUrl}/users/login/phone` : `${baseUrl}/users/resend-otp`
  ...
  const forgotPassword = async (identifier: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.message || `HTTP ${response.status}: Failed to send reset link`)
      }
    } catch (error) {
  ...
  const resetPassword = async (identifier: string, otp: string, newPassword: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.message || `HTTP ${response.status}: Failed to reset password`)
      }
    } catch (error) {
  ...
  const updateProfile = async (profileData: ProfileData) => {
    try {
      ...
      if (profileData.bank_name) formData.append("bank_name", profileData.bank_name)
